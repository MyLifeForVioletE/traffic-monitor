...
async function fetchAndRender() {
  ...
  const top1min = await getJSON(`/api/top-src?window_sec=${windowSec}&top_n=${topN}`);
  const topAll = await getJSON(`/api/top-src-all?top_n=${topN}`);
